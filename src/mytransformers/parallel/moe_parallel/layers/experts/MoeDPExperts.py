...
class DPExperts(MoeExperts):
    ...
    def forward(self,
        ...
        # ИЗМЕНЕНИЕ 1: Прямой синхронный вызов All-To-All вместо scheduler
        ...
        offsets = torch.zeros(num_local + 1, dtype=torch.long, device=hidden_states.device)
        torch.cumsum(counts, dim=0, out=offsets[1:])
        del counts
        ...
        # ИЗМЕНЕНИЕ 2: Прямой синхронный вызов All-To-All для возврата данных
        dist.all_to_all_single(
            output=result,
            input=recv_hidden,
            output_split_sizes=send_counts_list,
