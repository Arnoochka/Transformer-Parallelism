...
class MoeDPExpertsMemory(MoeExperts):
    ...
    def forward(self,
        ...
        for i in range(k):
            ...
            computed = torch.empty_like(hidden_states)
            ...
            computed.mul_(top_k_weights[:, i][local_sorted_indices].unsqueeze(-1))
            final_hidden_states.index_add_(0, local_sorted_indices, computed)
