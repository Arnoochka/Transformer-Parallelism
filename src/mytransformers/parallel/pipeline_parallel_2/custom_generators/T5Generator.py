...
class T5Generator(ParallelModuleGenerator):
    def __new__(cls,
        ...
        orig_modules = T5Generator.get_orig_modules(module)
        fake_modules = T5Generator.get_fake_modules(num_layers, device)
        ...
        inner_strategies = T5Generator.get_inner_strategies(len(inner_boundary_points))
        ...
        module = T5Generator.replace_modules(module, stage)
        modules = [module for name, module in stage.items()]
        fake_args = T5Generator.build_fake_args(num_layers, embed_size, vocab_size)
