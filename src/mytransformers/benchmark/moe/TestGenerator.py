...
from mytransformers.parallel.moe_parallel.pipeline import MoePipeline, BaseScheduler
from mytransformers.parallel.moe_parallel.generators import MoePipelineGenerator, MoeLayerConfig
from mytransformers.parallel.moe_parallel.layers import MoeSparseBlocFakekDPModule, MoeSparseBlockDPModule
...
class TestMoePipeGenerator(TestPipeGenerator):
    def __new__(cls,
        ...
        moe_mask = [is_moe for (name, orig_module, is_moe) in orig_modules]
        moe_layer_configs = cls.get_moe_layer_configs(orig_modules, num_experts, inner_boundary_points)
        dim_buffer = torch.empty(3, dtype=int, device=torch.cuda.current_device())
        
        stage: nn.ModuleDict = MoePipelineGenerator.get_stage(modules,
                                                              inner_boundary_points,
                                                              groups_info,
                                                              inner_comm_groups,
                                                              inner_strategies,
    ...
    def get_moe_layer_configs(orig_modules: List[Tuple[str, nn.Module, bool]],
        ...
        for idx, (name, module, is_moe) in enumerate(orig_modules):
            if is_moe:
                ...
                moe_layer_configs[name] = MoeLayerConfig(gate, main_rank, expert_idxs)
                
                if len(inner_boundary_points) > main_rank and idx == inner_boundary_points[main_rank]:
                    main_rank += 1
