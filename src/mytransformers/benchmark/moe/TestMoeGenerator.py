...
from mytransformers.parallel.moe_parallel.moe_pipeline.generators import PipelineGenerator
...
class TestMoeGenerator(ParallelModuleGenerator):
    def __new__(cls,
        ...
        orig_modules = TestMoeGenerator.get_orig_modules(module)
        fake_modules = TestMoeGenerator.get_fake_modules(num_layers, device)
        ...
        inner_strategies = TestMoeGenerator.get_inner_strategies(len(inner_boundary_points))
        ...
        module = TestMoeGenerator.replace_modules(module, stage)
        modules = [module for name, module in stage.items()]
        fake_args = TestMoeGenerator.build_fake_args(num_layers, embed_size, vocab_size)
