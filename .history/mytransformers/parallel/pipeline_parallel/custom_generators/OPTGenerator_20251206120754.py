...
class OPTGenerator(ParallelModuleGenerator):
    ...
    @staticmethod
    def replace_modules(module: OPTForCausalLM, pipe_modules: ModuleDict) -> Module:
        for name in ['embed_tokens', 'embed_positions', 'final_layer_norm']:
            setattr(module, name, pipe_modules[name])
        setattr(module, 'lm_head', pipe_modules['lm_head'])
        layers = ModuleList()
        for name, pipe_module in pipe_modules.items():
