    ...
    def __call__(self,
        ...
        for loop in range(self.loops):
            ...
            for batch in self.batches(prompts):
                output = model.generate(**batch, max_new_tokens=self.max_new_tokens, use_cache=False)
        ...
        for loop in range(self.loops):
            t_start = final_stats.loc[f"loop:{loop} start"]['time']
            t_stop  = final_stats.loc[f"loop:{loop} stop"]['time']
            inference_time = t_stop - t_start
            inference_times.append(inference_time)
            throughput = (self.batch_size * self.max_new_tokens) / inference_time
        ...
        for idx in range(world_size):
            ...
            max_memory_allocated_inference_per_device[idx] =\
                final_stats.loc[[f"loop:{loop} stop" for loop in range(self.loops)]][f"max_memory_gpu_{idx}"].max()
