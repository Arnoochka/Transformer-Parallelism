...
class TPSplittedLayerNormGenerator(TPModuleGenerator):
    """
    генерирует TPSplittedLayerNorm из torch.nn.LayerNorm
    ...
    def __new__(cls, module: LayerNorm, device: torch.device) -> TPSplittedLayerNorm:
        ...
        assert normalized_shape % tp_size == 0, \
            "normalized_shape must be divisible by tp_size"

        layer = TPSplittedLayerNorm(
            normalized_shape,
            cls.tp_group,
            module.eps,
...
class TPJoinedLayerNormGenerator(TPModuleGenerator):
    """
    генерирует TPJoinedLayerNorm из torch.nn.LayerNorm
