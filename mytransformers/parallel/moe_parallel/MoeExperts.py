...
class MoeExperts(ParallelModule):
    """
    Базовый класс для Mixture of Experts параллелизма

    Args:
        global_num_experts (int): Общее количество экспертов в распределённой среде.
        experts (ModuleList): Локальный список экспертов.
...
class MoeDPExperts(MoeExperts):
    """
    Реализация Mixture of Experts параллелизма с Data Parallel.

    Args:
        global_num_experts (int): Общее количество экспертов.
        local_experts (ModuleList): Локальный список экспертов.
        global_expert_idxs (Tensor): Индексы всех экспертов в глобальной системе.
