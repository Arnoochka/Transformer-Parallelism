...
if __name__ == "__main__":
    ...
    model = pp_custom.OPTGenerator(model, device)
    texts = [text for _ in range(64)]
    input_ids = tokenizer(texts, return_tensors="pt", max_length=256).to(device)
    TRACKER = init_global_tracker()
    TRACKER.start()
    output = model(**input_ids, use_cache=False)
    TRACKER.snapshot("complete")
    utils.Logger.log_all_device(output)    
    df = TRACKER.stop()
